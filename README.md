...
The web app is built for Vercel with `npm run build:vercel`; the build emits a static entry page and Vercel Build Output API artifacts. The Vercel project should use the `Other` framework preset, keep the repository root as the project root, and use the committed `vercel.json` build command. The CDN serves the UI and the checked-in snapshot without a database connection or runtime server function. The metadata URL is fixed to `https://cardtape.bimlabs.xyz`; no `SITE_URL` environment variable is needed.
